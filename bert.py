...
from tensorflow.keras.layers import Dropout, Input, Sequential, Dense, Embedding, GlobalAveragePooling1D, LayerNormalization, MultiHeadAttention
...
def transformer_block(embed_dim, num_heads, i, ff_dim, rate=0.1):
    ...
    outputs = LayerNormalization(epsilon=1e-6)(out1 + ffn_output)
    model = Model(inputs, outputs, name=f"TransformerBlock_{i}")
    model.summary()
    return model


def create_bert_model(num_heads=4, num_blocks=2, feedforward_dim=128):
    ...
    for i in range(num_blocks):
        print(i)
        model.add(transformer_block(128, num_heads, i, feedforward_dim))
...
def save_best_results(best_params,
    ...
    final_model = create_bert_model(num_heads=best_params["num_heads"],
                            num_blocks=best_params["num_blocks"],
                            feedforward_dim=best_params["feedforward_dim"])

    history = final_model.fit(X_train_pad, y_train, validation_data=(X_val_pad, y_val), epochs=15, batch_size=32)
...
df = sample_dataframe(df, total_samples=25000)
...
model = create_bert_model(num_heads=4, num_blocks=2, feedforward_dim=128)
# Train Model
history = model.fit(
    X_train_pad, y_train,
...
best_model, best_params, best_score = grid_search(create_bert_model, param_grid, X_train_pad, y_train, X_val_pad, y_val, epochs=15, batch_size=32)
