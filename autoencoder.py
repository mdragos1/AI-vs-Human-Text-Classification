...
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Input, LSTM, Dense, Embedding, RepeatVector, Dropout, BatchNormalization, TimeDistributed
...
def create_model(embedding_size=256, lstm_size=256, dropout_rate=0.5):
    '''Create autoencoder model'''
    input_text = Input(shape=(max_length,))
    embedded = Embedding(vocab_size, embedding_size)(input_text)
    encoded = LSTM(lstm_size, activation='tanh', return_sequences=False)(embedded)
    ...
    # Autoencoder Model
    autoencoder = Model(input_text, output)
    ...
    # Ensure y_train is reshaped to match the required output shape
    y_train_pad = np.expand_dims(X_train_pad, axis=-1)
    y_val_pad = np.expand_dims(X_val_pad, axis=-1)
    ...
    encoded_features = encoder.predict(X_train_pad)
    encoded_features_val = encoder.predict(X_val_pad)

    # Check Unique Encodings
    unique_values = np.unique(encoded_features, axis=0)
    unique_values_val = np.unique(encoded_features_val, axis=0)
    print(f"Number of unique feature vectors: {len(unique_values)}")
    print(f"Number of unique val feature vectors: {len(unique_values_val)}")
...
def grid_search(model_function, param_grid, X_train, y_train, X_val, y_val, epochs=25, batch_size=32):
...
df = sample_dataframe(df, total_samples=5000)
...
model = create_model(embedding_size=256, lstm_size=256, dropout_rate=0.5)
# Train Model
history = model.fit(
    X_train_pad, y_train,
...
best_model, best_params, best_score = grid_search(create_model, param_grid, X_train_pad, y_train, X_val_pad, y_val, epochs=25, batch_size=32)
